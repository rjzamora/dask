...
_funcs = {M.sum: "sum", M.count: "count", M.mean: "mean"}


def _apply_multi_pass(df, funcs, **kwargs):
...
def _apply_single_pass(df, funcs, **kwargs):
    cols = []
    aggs = {}
    new_names = {}
    for name, _, agg in funcs:
        col = agg["column"]
        func = _funcs.get(agg["func"], agg["func"])
        if col in aggs:
            new_names[col].append(name)
            aggs[col].append(func)
        else:
            cols.append(col)
            new_names[col] = [name]
            aggs[col] = [func]

    result = _groupby_raise_unaligned(df, **kwargs).agg(aggs)
    names = []
    for col in cols:
        names += new_names[col]
...
def _groupby_apply_funcs(df, *by, **kwargs):
    ...
    # Single-pass version (like dask_cudf)
    single_pass = False
    if single_pass and is_dataframe_like(df) and len(funcs) > 1:
        return _apply_single_pass(df, funcs, **kwargs)

    return _apply_multi_pass(df, funcs, **kwargs)
...
class _GroupBy:
    ...
    def aggregate(self, arg, split_every, split_out=1):
        ...
        if isinstance(self.obj, DataFrame):
            aggs = _redirect_aggs(arg)
            if use_optimized_path and _aggs_supported(aggs, SUPPORTED_AGGS):
                return optimized_groupby_agg(
                    self.obj,
                    self.by,
                    aggs,
                    split_every=split_every,
        ...
        return aca(
            ...
            chunk_kwargs=dict(
                funcs=chunk_funcs,
                sort=False if self.obj.npartitions > 1 else self.sort,
                **self.observed,
                **self.dropna,
            ),
...
def optimized_groupby_agg(
    ...
    return aca(
        ...
        chunk_kwargs={
            ...
            "columns": columns,
            "dropna": dropna,
            "sort": False if ddf.npartitions > 1 else sort,
            "sep": sep,
        },
...
def _groupby_supported(gb):
    ...
    return isinstance(gb.obj, DataFrame) and (
...
def _var_agg_opt(df, col, count_name, sum_name, pow2_sum_name, ddof=1):
    ...
    for col in columns:
        ...
        if agg_set.intersection({"mean", "std", "var"}):
            ...
            if agg_set.intersection({"std", "var"}):
                ...
                var = _var_agg_opt(gb, col, count_name, sum_name, pow2_sum_name)
                if "var" in agg_list:
